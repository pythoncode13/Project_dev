...
class AdvancedTradeAnalysis:
    def __init__(self, analysis_advanced):
        ...
        self.t1down = self.model.t1down
        self.t2down = self.model.t2down
        self.t3down = self.model.t3down
        self.CP_down_point = self.model.CP_down_point
        self.down_enter_point = self.model.down_enter_point
        self.point_t1_t2 = self.model.point_t1_t2
        self.max_distance = self.model.point_t1_t2
        self.intersection_t1_point_t2_t3_lines = self.model.intersection_t1_point_t2_t3_lines
    ...
    def calculate_additional_properties(self):
        ...
        """
        points_i = [self.t1down, self.t2down, self.t3down, self.down_enter_point]
        t1down_norm, t2down_norm, t3down_norm, down_enter_point_norm = normalize_points(
            points_i)

        price_change_t1_t2 = (self.t2down[1] - self.t2down[1]) / self.t2down[1]
        price_change_t2_t3 = (t3down_norm[1] - self.t2down[1]) / self.t2down[1]

        diff_price_change = abs(price_change_t1_t2 - price_change_t2_t3)

        points_i = [self.CP_down_point, self.t1down, self.t2down, self.t3down,
                    self.down_enter_point, self.point_t1_t2,
                    self.intersection_t1_point_t2_t3_lines]
        (CP_down_point_norm, t1down_norm, t2down_norm, t3down_norm,
        down_enter_point_norm, point_t1_t2_norm,
         intersection_t1_point_t2_t3_lines_norm) = normalize_points(points_i)

        length3_point_t1_t2_norm = calculate_distance(self.t1down,
                                                      intersection_t1_point_t2_t3_lines_norm)
        angle_t2_t3 = math.atan2(self.t3down[1] - self.t2down[1],
                                 self.t3down[0] - self.t2down[0])
        area_under_curve = 0.5 * (self.t3down[0] - self.t1down[0]) * (
                    self.t1down[1] + self.t3down[1])

        return diff_price_change, length3_point_t1_t2_norm, angle_t2_t3, area_under_curve

    @property
    def calculate_param_t2_t3_down_enter(self):
        """
        Вычисляет угол и радиус кривизны между заданными точками.
        """
        t2down = np.array(self.t2down)
        t3down = np.array(self.t3down)
        down_enter_point = np.array(self.down_enter_point)

        angle_t3_enter = math.atan2(down_enter_point[1] - t3down[1],
                                   down_enter_point[0] - t3down[0])
        radius_curvature_t2_t3_enter = abs(
            (t3down[1] - t2down[1]) * (down_enter_point[0] - t3down[0]) - (
                        t3down[0] - t2down[0]) * (
    ...
    def compute_std_dev_y_mean_y(self):
        ...
        _, self.std_dev_y_mean_y = compute_stats(self.t1down, self.t2down, self.t3down)
        _, self.std_dev_y_mean_y_1 = compute_stats(self.t1down, self.t2down, self.t3down, self.down_enter_point)
        _, self.std_dev_y_mean_y_2 = compute_stats(self.t1down,
                                              self.intersection_t1_point_t2_t3_lines,
                                              self.point_t1_t2)
        _, self.std_dev_y_mean_y_3 = compute_stats(self.t1down, self.t2down, self.t3down, self.down_enter_point,
                                              self.intersection_t1_point_t2_t3_lines,
                                              self.point_t1_t2)
        _, self.std_dev_y_mean_y_4 = compute_stats(self.t1down, self.t2down, self.t3down)
        _, self.std_dev_y_mean_y_5 = compute_stats(self.t1down, self.t2down, self.down_enter_point)
        _, self.std_dev_y_mean_y_6 = compute_stats(self.t1down, self.t3down,
                                              self.intersection_t1_point_t2_t3_lines)
        _, self.std_dev_y_mean_y_7 = compute_stats(self.t2down, self.down_enter_point, self.point_t1_t2)
        self.std_dev_y_t2_t3_down_enter, _ = compute_stats(self.t2down, self.t3down, self.down_enter_point)
        ...
        return (
            ...
            self.std_dev_y_t2_t3_down_enter
        )

    @property
    def get_rsi(self):
        self.rsi_value1 = self.df.loc[self.t1down[0], 'rsi']
        self.rsi_value2 = self.df.loc[self.t2down[0], 'rsi']
        self.rsi_value3 = self.df.loc[self.t3down[0], 'rsi']
        self.rsi_value_enter = self.df.loc[self.down_enter_point[0], 'rsi']
    ...
    @property
    def get_vwap(self):
        self.vwap_t1 = self.df.loc[self.t1down[0], 'vwap']
        self.vwap_enter = self.df.loc[self.down_enter_point[0], 'vwap']
        self.vwap_ratio_t1 = self.t1down[1] / self.vwap_t1
        self. vwap_ratio_enter = self.down_enter_point[1] / self.vwap_enter

        self.vwap_t1_v2 = self.df.loc[self.t1down[0], 'vwap2']
        self.vwap_enter_v2 = self.df.loc[self.down_enter_point[0], 'vwap2']
        self.vwap_ratio_t1_v2 = self.t1down[1] / self.vwap_t1
        self.vwap_ratio_enter_v2 = self.down_enter_point[1] / self.vwap_enter
