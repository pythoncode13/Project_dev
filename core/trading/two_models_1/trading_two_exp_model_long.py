...
def prepare_trading_setup(super_groups, ticker):
    ...
    for two_model_group in super_groups:
        ...
        # lower_body_edge = up.df['high']
        #
        # # Задаем диапазон поиска
        # lower_limit = int(down_LT_break_point[0] + 1)
        # upper_limit = min(up.properties.dist_cp_t4_x1, len(up.df))
        #
        ...
        upper_limit = min(int(down_LT_break_point[0]) + 110, len(up.df))
        ...
        entry_index = int(down_LT_break_point[0])
        ...
        take_price = entry_price + 0.9 * (
                down.properties.target_3 - entry_price)
        print(take_price)
