...
def process_group_long(df, up_trend_points, ticker):
    ...
    for model in up_trend_points:
        ...
        bars_above_t4up = np.where(
            lower_body_edge[lower_limit:upper_limit] > model.up_take_lines[1]
        )
        ...
        entry_price = df.loc[first_bar_above_t4up, 'close']
        # 4stop_price = t1up[1]

        stop_price = CP_up_point[1]
        # up_take_lines = analysis_base.up_take_lines

        up_take_200 = t4up[1] + (t4up[1] - t1up[1]) * 1

        take_price = up_take_200
        ...
        plt.text(
            ...
            y=take_price,  # координата y для размещения текста
            s='Тейк',  # сам текст
            color='g',  # цвет текста
        ...
        if percent_difference < 0.4 or risk_reward_ratio < 0.5:
            continue
